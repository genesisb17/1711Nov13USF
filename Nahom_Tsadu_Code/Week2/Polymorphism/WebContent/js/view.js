...
let scrollReqs = {
		...
		[attributes.overloading] : {
				...
				alternateScroll: ()=>{return (($(window).height() - $(map[getPrevious()]).height()) / 8).toString();}
			},
...
function setView(attribute){ 
	if(getCurrent() == attribute){ 
		...
		if(getPrevious() == null || getPrevious() == "home")$('html, body').animate({scrollTop: scrollReqs[attribute].normalScroll() + "px"}, 800);   
