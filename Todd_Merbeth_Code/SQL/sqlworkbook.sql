...
CREATE OR REPLACE PROCEDURE newCustomer(
    ...
    com in CUSTOMER.COMPANY%TYPE,
    ad in CUSTOMER.ADDRESS%TYPE,
    city in CUSTOMER.CITY%TYPE,
    s in CUSTOMER.STATE%TYPE,
    cn in CUSTOMER.COUNTRY%TYPE,
    pc in CUSTOMER.POSTALCODE%TYPE,
    ph in CUSTOMER.PHONE%TYPE,
    fax in CUSTOMER.FAX%TYPE,
    email in CUSTOMER.EMAIL%TYPE,
    srid in CUSTOMER.SUPPORTREPID%TYPE
    )
AS
BEGIN
  INSERT INTO CUSTOMER VALUES (cid, fn, ln, com, ad, city, s, cn, pc, ph, fax, email, srid);
END;
/

DECLARE
  newCust CUSTOMER%ROWTYPE;
BEGIN
  newCustomer('62', 'Todd', 'Merbeth', 'Revature', '1111 Livingston', 'Tampa', 'FL', 'US', '11111', '+1 (551) 555-5555', '+1 (551) 555-5555', 'toddmerbeth@gmail.com', '5');
END;
/
--6.0
CREATE OR REPLACE TRIGGER emp_In_Trig
AFTER INSERT ON EMPLOYEE
FOR EACH ROW
BEGIN
    dbms_output.put_line('EMP_IN_TRIG SET OFF');
END;
/
insert into employee values('11', 'Trig', 'Triggo', 'Trigger Man', '6', '11-JUN-75', '15-May-05', '222 Two St', 'Calgary', 'AB', 'Canada', 'T2P 4L2', '+1 (121) 112-2112', '+1 (112) 212-2873', 'trig@chinookcorp.com');

CREATE OR REPLACE TRIGGER al_Up_Trig
AFTER UPDATE ON ALBUM
FOR EACH ROW
BEGIN
    dbms_output.put_line('AL_UP_TRIG SET OFF');
END;
/
UPDATE ALBUM SET TITLE='TRIG' WHERE ALBUMID = 1;

CREATE OR REPLACE TRIGGER cust_Del_Trig
AFTER DELETE ON CUSTOMER
FOR EACH ROW
BEGIN
    dbms_output.put_line('CUST_DEL_TRIG SET OFF');
END;
/
...
--7.2
SELECT cus.CUSTOMERID, cus.FIRSTNAME, cus.LASTNAME, inv.INVOICEID, inv.TOTAL FROM CUSTOMER cus FULL OUTER JOIN INVOICE inv ON cus.CUSTOMERID = inv.CUSTOMERID;

--7.3
SELECT art.NAME, alb.TITLE FROM ALBUM alb RIGHT JOIN ARTIST art ON alb.ARTISTID = art.ARTISTID;

--7.4
SELECT * FROM ALBUM al CROSS JOIN ARTIST art WHERE al.ARTISTID = art.ARTISTID ORDER BY art.NAME ASC;
